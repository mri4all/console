...
def write_dicom(image_ndarray, task, folder):
    ...
    for echo_id in range(ndarray_dims[-1]):
        for slc_id in range(ndarray_dims[-2]):
            ...
            dicom_hdr.SOPInstanceUID = dicom_hdr.SeriesInstanceUID + '.' + str(instance_counter)
            ...
            dicom_filename = dicom_hdr.SeriesInstanceUID + '#' + str(dicom_hdr.InstanceNumber) + '.dcm'
...
def set_patient_information(dicom_dataset, PatientInformation):
    ...
    dicom_dataset.AccessionNumber = PatientInformation.acc 
