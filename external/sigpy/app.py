...
from sigpy.alg import PowerMethod, GradientMethod, ADMM, ConjugateGradient, PrimalDualHybridGradient
...
class MaxEig(App):
    ...
    def __init__(self, A, dtype=float, device=backend.cpu_device, max_iter=30, show_pbar=True, leave_pbar=True):
...
class LinearLeastSquares(App):
    ...
    def _summarize(self):
        ...
        if self.show_pbar:
            if self.save_objective_values:
                ...
                self.pbar.set_postfix(resid="{0:.2E}".format(backend.to_device(self.alg.resid, backend.cpu_device)))
    ...
    def _get_ConjugateGradient(self):
        ...
        self.alg = ConjugateGradient(AHA, AHy, self.x, P=self.P, max_iter=self.max_iter, tol=self.tol)
    ...
    def _get_GradientMethod(self):
        ...
        if self.alpha is None:
            ...
            max_eig = MaxEig(
                AHA, dtype=self.x.dtype, device=self.x_device, max_iter=self.max_power_iter, show_pbar=self.show_pbar
            ).run()
            if max_eig == 0:
                self.alpha = 1
            else:
    ...
    def _get_PrimalDualHybridGradient(self):
        ...
        if self.tau is None:
            ...
            max_eig = MaxEig(
                AAH, dtype=self.x.dtype, device=self.x_device, max_iter=self.max_power_iter, show_pbar=self.show_pbar
            ).run()
    ...
    def _get_ADMM(self):
        ...
        def minL_x():
            ...
            App(ConjugateGradient(AHA, AHy, self.x, P=self.P, max_iter=self.max_cg_iter), show_pbar=False).run()
        ...
        self.alg = ADMM(minL_x, minL_v, self.x, v, u, G, -I_v, 0, max_iter=self.max_iter)
    ...
    def objective(self):
        with self.y_device:
            ...
            if self.lamda > 0:
                if self.z is None:
                    obj += self.lamda / 2 * self.x_device.xp.linalg.norm(self.x).item() ** 2
                else:
                    obj += self.lamda / 2 * self.x_device.xp.linalg.norm(self.x - self.z).item() ** 2

            if self.proxg is not None:
                if self.g is None:
                    raise ValueError("Cannot compute objective when proxg is specified," "but g is not.")
...
class L2ConstrainedMinimization(App):
    ...
    def __init__(self, A, y, proxg, eps, x=None, G=None, max_iter=100, tau=None, sigma=None, show_pbar=True):
        self.y = y
        self.x = x
        self.y_device = backend.get_device(y)
        if self.x is None:
        ...
        self.x_device = backend.get_device(self.x)
        if G is None:
            self.max_eig_app = MaxEig(A.N, dtype=self.x.dtype, device=self.x_device, show_pbar=show_pbar)
        ...
        if tau is None or sigma is None:
            max_eig = MaxEig(A.H * A, dtype=self.x.dtype, device=self.x_device, show_pbar=show_pbar).run()
            tau = 1
            sigma = 1 / max_eig
        ...
        alg = PrimalDualHybridGradient(proxfc, proxg, A, A.H, self.x, self.u, tau, sigma, max_iter=max_iter)
